...
@Injectable()
export class SystemMapShopTaskCompareBusiness {
  constructor(private service: ArmAnalysisRequestService) {}

  async load(task: SystemMapTaskShopArgs, args: SystemMapShopArgs) {
    let one = await this.data(task.ids[0], args);
    let two = await this.data(task.ids[1], args);
  ...
  private data(taskId: string, args: SystemMapShopArgs) {
