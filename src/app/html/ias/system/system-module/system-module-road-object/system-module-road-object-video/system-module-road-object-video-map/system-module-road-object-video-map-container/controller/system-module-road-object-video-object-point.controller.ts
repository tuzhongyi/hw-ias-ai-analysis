...
export class SystemMainMapRoadObjectPointController {
  inited = false;
  event = {
  ...
  private regist(subscription: Subscription) {
    let sub1 = this.amap.event.road.object.point.click.subscribe((x) => {
      this.event.click.emit(x);
    });
    subscription.add(sub1);
    let sub2 = this.amap.event.road.object.point.dblclick.subscribe((x) => {
