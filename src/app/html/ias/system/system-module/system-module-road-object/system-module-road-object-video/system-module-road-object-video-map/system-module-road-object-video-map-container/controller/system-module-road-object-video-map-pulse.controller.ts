...
import { SystemModuleRoadObjectVideoAMapController } from './system-module-road-object-video-amap.controller-pulse';
...
export class SystemModuleRoadObjectVideoMapController {
  ...
  on = {
    label: {
      show: async (data: {
        ...
        let line = {
          start: this.path.datas.flat(1).find((x) => {
            return ClassTool.equals.array(
              [x.Longitude, x.Latitude],
              data.line[0]
            );
          }),
          end: this.path.datas.flat(1).find((x) => {
            return ClassTool.equals.array(
              [x.Longitude, x.Latitude],
              data.line[1]
            );
  ...
  private async onclick(data: {
    ...
    let datas = this.path.datas.flat(1);
    let line = {
      start: datas.find((x) => {
