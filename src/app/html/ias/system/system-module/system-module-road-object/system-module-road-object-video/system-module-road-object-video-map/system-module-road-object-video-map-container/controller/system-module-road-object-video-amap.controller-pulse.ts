...
import { IASMapAMapPathPulseController } from '../../../../../../../share/map/controller/amap/path/ias-map-amap-path-pulse.controller';
...
export class SystemModuleRoadObjectVideoAMapController {
  ...
  private init = {
    ...
    path: (map: AMap.Map, container: Loca.Container) => {
      ...
      this._path.set(new IASMapAMapPathPulseController(map, container));
    },
    pickup: {
    ...
    roadobject: {
      ...
      marker: (map: AMap.Map, info: IASMapAMapInfoController) => {
        ...
        let sub1 = ctr.event.click.subscribe(async (x) => {
          this.event.road.object.point.click.emit(x);

          let gcj02 = x.Location.GCJ02;
          let point: GeoPoint = [gcj02.Longitude, gcj02.Latitude];
          let path = await this._path.get();

          let closests = path.datas.map((x) => {
            return GeoTool.polyline.closest.get(x, point);
          });
