...
import { SystemModuleShopRegistrationMapAMapConfig } from '../system-module-shop-registration-map-amap.config';

@Injectable()
export class SystemModuleShopRegistrationMapAMapRoadPolylineController {
  ...
  private create(datas: [number, number][]): AMap.Polyline {
    const polyline = new AMap.Polyline({
      path: datas,
      strokeColor: SystemModuleShopRegistrationMapAMapConfig.road.color.normal,
      strokeWeight: 5,
      strokeOpacity: 0.5,
      // 折线样式还可以为'dashed'
      strokeStyle: 'solid',
  ...
  select(id: string) {
    ...
    if (line) {
      ...
      line.setOptions({
        strokeColor:
          SystemModuleShopRegistrationMapAMapConfig.road.color.selected,
        strokeOpacity: 0.8,
      });
    }
    return line;
  ...
  blur() {
    if (this.selected) {
      ...
      if (line) {
        line.setOptions({
          strokeColor:
            SystemModuleShopRegistrationMapAMapConfig.road.color.normal,
          strokeOpacity: 0.5,
        });
      }
