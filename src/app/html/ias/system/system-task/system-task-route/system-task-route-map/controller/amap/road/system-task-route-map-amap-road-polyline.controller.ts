...
import { SystemTaskRouteMapAMapConfig } from '../system-task-route-map-amap.config';

@Injectable()
export class SystemTaskRouteMapAMapRoadPolylineController {
  ...
  private create(datas: [number, number][]): AMap.Polyline {
    const polyline = new AMap.Polyline({
      path: datas,
      strokeColor: SystemTaskRouteMapAMapConfig.road.color.normal,
      strokeWeight: 5,
      strokeOpacity: 0.5,
      // 折线样式还可以为'dashed'
      strokeStyle: 'solid',
  ...
  select(id: string) {
    ...
    if (line) {
      this.selected = id;
      line.setOptions({
        strokeColor: SystemTaskRouteMapAMapConfig.road.color.selected,
        strokeOpacity: 0.8,
      });
    }
    return line;
