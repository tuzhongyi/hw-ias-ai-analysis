...
import { SystemMainCardEventChartBarEChartOption } from './system-main-card-event-chart-bar-container-echart.option';

@Component({
  selector: 'ias-system-main-card-event-chart-bar-container',
  imports: [],
  templateUrl: './system-main-card-event-chart-bar-container.component.html',
  styleUrl: './system-main-card-event-chart-bar-container.component.less',
})
export class SystemMainCardEventChartBarContainerComponent
  extends ChartAbstract
  implements OnInit, OnChanges, AfterViewInit, OnDestroy
{
  @Input() option = Object.assign({}, SystemMainCardEventChartBarEChartOption);
  ...
  @ViewChild('system_main_card_event_chart_bar_container_chart')
  element?: ElementRef;
  ...
  private set = {
    color: (colors: (IChartColor | undefined)[]) => {
      ...
      for (let i = 0; i < colors.length; i++) {
        ...
        if (color.line) {
          this.option.series[i].barStyle.color = color.line;
        }
        if (color.point) {
